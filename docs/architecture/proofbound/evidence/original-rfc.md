# Original RFC — pre-implementation design intent (§§1–24)

> **Historical.** This is the RFC as written *before* any implementation, preserved for its reasoning.
> **It is not authoritative.** Where it disagrees with the normative documents, they win — and in several
> places it is now known to be wrong (the fix-loop model, `dsd_spec.py`, `specs/<id>/manifest.json`, the
> `-text` suggestion, the artifact-kind DAG). Read it to understand *why* a decision was considered, never
> to discover *what* the rule is.
>
> Corrections are recorded in [implementation-findings.md](implementation-findings.md). Current
> architecture starts at [../README.md](../README.md).
>
> Section numbers below are the original ones and are referenced from elsewhere as stable identifiers.
