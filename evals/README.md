...
| **Can the workflow make the right decision and deliver the required result?** | Calibrated valid and invalid cases, real-agent observations, external checks of the artifacts produced | Partly. Invalid cases: [Eval V1](eval-v1.md) (planted contradictions) and two authority demonstrations, both of which correctly **stopped**. Valid case: one live continuation from **seeded** upstream authority, `pb-handoff-1` ([report](authority_slice/runs/pb-handoff-1/run-report.md)) and again on the admission-repaired instrument as `pb-handoff-2` ([report](authority_slice/runs/pb-handoff-2/run-report.md)); one observation per condition. The goal-to-change workflow has run against a real agent **once**: the public CSV task on 2026-09-24, an accepted delivery verified on a fresh checkout, with a manually assisted handoff ([record](../docs/architecture/proofbound/evidence/qualification-and-first-use-2026-09-24.md)). Configuration qualification (`pb_qualify.py`) has live DeepSeek observations, one per cell; the contradiction case was verification with author exposure, not independent discovery. No comparison has been run |
