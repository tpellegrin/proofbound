...
2. **A tenant's own work keeps its order.** Within one key, items are dispatched in the order they
   arrived. A tenant that enqueues `x` then `y` never sees `y` served first.
