...
def deliver(notification) -> dict:
    ...
    while attempts < MAX_ATTEMPTS:
        ...
        # Beacon answers 200 whenever it accepted the request for processing; what it did with
        # the message is in the body.
        result = body.get("result")
        if result == "accepted":
            return {"status": "sent", "attempts": attempts, "message_id": body.get("id")}
        if result == "refused":
            return {"status": "rejected", "attempts": attempts, "message_id": None}
    return {"status": "failed", "attempts": attempts, "message_id": None}


registry.register("beacon", deliver)
