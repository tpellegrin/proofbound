...
def spend(run_root: "str | Path", db: "str | Path", *, model: str = MODEL,
          ...
          limit: float = 0, reserve: float = 0) -> "dict[str, Any]":
    ...
    priced = _pricing.cost_rows(events["rows"], model=model.split("/", 1)[-1])
...
class LaunchLedger:
    ...
    def __init__(self, path: "str | Path", *, limit: float = 0, reserve: float = 0,
                 ceiling: int = 0, repair_cycles: int = 1) -> None:
        self.path = Path(path)
    ...
    def admit(self, *, phase: str, task: str, role: str, run_root: "str | Path",
        ...
        if account["complete"] and derived + reserve > limit:
